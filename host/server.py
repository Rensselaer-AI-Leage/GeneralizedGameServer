...
class Server:
	...
	def __init__(self, game, settings):
		...
		# Error log
		log_dir = os.path.join('logs', 'act')
		err_dir = os.path.join('logs', 'err')
		self.log_file = os.path.join(log_dir, str(datetime.datetime.now()) + ".log")
		self.err_file = os.path.join(err_dir, str(datetime.datetime.now()) + ".log")
		self.log_file = self.log_file.replace(':', '-')
		self.err_file = self.err_file.replace(':', '-')
		if not os.path.isdir(log_dir): os.makedirs(log_dir)
		if not os.path.isdir(err_dir): os.makedirs(err_dir)
